"""
Deep Capsule Network Architecture Pytorch Implementation
"""
...
def squash(caps, dim=-1, eps=1e-8):
    """
    CapsNet activation function for non-linearity
    """
...
class Conv2DCaps(nn.Module):
    ...
    def __init__(
        ...
        self.conv = nn.Conv2d(
            in_channels=reshaped_in_channels,
...
class Conv3DCaps(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(
            batch_size,
            self.conv_channel_in,
            self.conv_channel_out,
            self.caps_num_out,
            self.height,
            self.width,
        )
    ...
    def routing(self, x, x_detached, batch_size, routing_iter=3):
        ...
        for iter_idx in range(routing_iter):
            ...
            temp = (
                self.B.permute(0, 5, 3, 1, 2, 4)
                .contiguous()
                .view(
                    batch_size,
                    self.conv_channel_in,
                    1,
                    self.height * self.width * self.conv_channel_out,
                )
            )
...
class FC_Caps(nn.Module):
    def __init__(
        ...
        self.b = nn.Parameter(
            torch.randn(1, 1, self.output_capsules, self.out_dimensions) * 0.05
        )
    ...
    def forward(self, x):
        ...
        b_ij = x.new_zeros(
            size=(x.size()[0], self.input_capsules, self.output_capsules, 1),
            requires_grad=False,
        ).to(self.device)
        ...
        for iter_idx in range(self.routing_iter):
            ...
            if iter_idx == self.routing_iter - 1:
                s_j = (c_ij * u_hat).sum(
                    dim=1, keepdim=True
                ) + self.b  # multiply with the original u_hat since we want the gradient flow.
...
class Mask_CID(nn.Module):
    ...
    def forward(self, x, target=None):
        ...
        batch_ind = torch.arange(start=0, end=batch_size).to(
            self.device
        )  # a tensor containing integer from 0 to batch size.
        m = torch.stack([batch_ind, max_len_indices], dim=1).to(
            self.device
...
class Decoder(nn.Module):
    ...
    def __init__(self, caps_dimension, device, num_caps=1, img_size=28, img_channels=1):
        ...
        self.fc = nn.Sequential(
            torch.nn.Linear(
                self.caps_dimension * self.num_caps, self.neurons * self.neurons * 16
            ),
            nn.ReLU(inplace=True),
        ).to(self.device)

        self.reconst_layers1 = nn.Sequential(
        ...
        self.reconst_layers4 = nn.Sequential(
            nn.ConvTranspose2d(
...
class DeepCapsModel(nn.Module):
    ...
    def __init__(self, num_class, img_height, img_width, device):
        ...
        self.fc_caps = FC_Caps(
            output_capsules=self.num_class,
            input_capsules=640,
            in_dimensions=8,
            out_dimensions=16,
            routing_iter=3,
    ...
    def forward(self, x, y, target=None):
        ...
        """
        stream_1 = self.stream(y)
        stream_2 = self.stream(y)
        
        dig_caps = self.fc_caps(stream_1)
        x = self.to_scalar(dig_caps)
        dig_caps1 = self.fc_caps(stream_2)
        x1 = self.to_scalar(dig_caps1)

        masked, indices = self.mask(dig_caps, target)
        decoded = self.decoder(masked)
        masked1, indices1 = self.mask(dig_caps1, target)
        decoded1 = self.decoder(masked1)
        return dig_caps, masked, decoded, indices, dig_caps1, masked1, decoded1, indices1
    ...
    def margin_loss(self, x, labels, lambda_, m_plus, m_minus):
        ...
        classification_loss = (labels * max_l + lambda_ * (1 - labels) * max_r).sum(
            dim=1
        )
    ...
    def reconstruction_loss(self, reconstructed, data):
        ...
        loss = self.mse_loss(
            reconstructed.view(batch_size, -1), data.view(batch_size, -1)
        )
        return 0.4 * loss.sum(dim=1)

    def loss(
        self, x, reconstructed, data, labels, lambda_=0.5, m_plus=0.9, m_minus=0.1
    ):
        """
        Mean of total loss calculation. Both reconstruction loss and classification loss.
        """
        total_loss = self.margin_loss(
            x, labels, lambda_, m_plus, m_minus
        ) + self.reconstruction_loss(reconstructed, data)
