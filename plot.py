...
def plot_loss_acc(path, num_epoch, train_accuracies, train_losses, test_accuracies, test_losses):
    ...
    data = pd.concat([train_accuracy_df, test_accuracy_df], ignore_index= True)
    ...
    data = pd.concat([train_loss_df, test_loss_df], ignore_index= True)
